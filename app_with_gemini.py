...
class ChurnPredictionApp:
    ...
    def load_and_prepare_data(self):
        ...
        try:
            ...
            feature_importance = pd.DataFrame(
                {"feature": model_features, "importance": self.model.feature_importances_}
    ...
    def generate_general_insights(self):
        ...
        try:
            ...
            feature_ranges_str = "\n".join(
                [
                    f"- {feature}: Min = {ranges['min']:.2f}, Max = {ranges['max']:.2f}, Mean = {ranges['mean']:.2f}"
                    for feature, ranges in self.data_analysis["feature_ranges"].items()
                ]
            )

            prompt = f"""Provide in-depth insights about the dataset:
            - Total Customers: {len(self.data_analysis['data'])}
            - Churn Rate: {self.data_analysis['data']['churn'].mean()*100:.2f}%
            - Top Features: {', '.join(self.data_analysis['top_features'])}
    ...
    def generate_personalized_recommendations(self, features, churn_probability):
        ...
        try:
            ...
            # Determine recommendations based on user context
            recommendations = []
            prompt = f"""Personal Churn Risk Analysis:
    ...
    def create_feature_inputs(self):
        """Create feature inputs with ranges from the dataset"""
        features = {
        ...
        with tab2:
            ...
            if st.button('Predict Churn Risk'):
                try:
                    ...
                    # Generate personalized recommendations immediately after prediction
